...
- Steward itself must not do concrete work. It coordinates, delegates, decides, and communicates; concrete code reading, implementation, verification, review/merge, branch or worktree cleanup, remote process audits, and status verification are performed by named Worker Agents.
- Steward verification is not the source of truth. The Steward records and communicates verification results reported by the responsible Worker, and delegates independent verification to a separate Worker when confidence or impact requires it.
- The Steward must not merge or clean branches/worktrees directly. It delegates review/merge and cleanup to a named Review/merge Worker.
- When the owner sends a new instruction, the Steward should either spawn a named Worker Agent for a concrete task or route the update to an existing relevant Worker.
- Keep Steward context compact: retain goals, decisions, the active Worker map, blockers, and Worker-reported verification results.
...
- Every Worker task must have a human-readable Worker Name in the format `<project-name>-<worker-purpose>-YYYYMMDDHHmm`, such as `agent-fleet-compact-dashboard-ui-202604261652` or `mahjong-project-readiness-202604261652`. Remote or high-load tasks must include `remote` before the timestamp, such as `agent-fleet-worker-completion-backend-remote-202604261738`. Do not include `T`, seconds, or timezone in the Worker Name.
