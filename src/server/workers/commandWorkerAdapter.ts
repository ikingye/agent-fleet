...
function startWorkerProcess(input: StartWorkerProcessInput): Promise<WorkerStartResult> {
  return new Promise((resolve) => {
    ...
    child.stdin.on("error", (error) => {
      ...
      const extraOutput = `\nWorker prompt write failed: ${error.message}`;
      settle("failed", extraOutput);
      settleCompletion("failed", extraOutput);
    });
    child.on("close", (code, signal) => {
      const settleClosedProcess = () => {
        if (stdinWriteFailed) {
          return;
        }

        if (!stdinSettled) {
          const extraOutput = "\nWorker prompt write failed: Worker process exited before accepting stdin";
          settle("failed", extraOutput);
          settleCompletion("failed", extraOutput);
          return;
        }
      ...
      setImmediate(settleClosedProcess);
    });
    try {
      ...
      const extraOutput = `\nWorker prompt write failed: ${message}`;
      settle("failed", extraOutput);
      settleCompletion("failed", extraOutput);
    }
  });
