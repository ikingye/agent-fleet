...
import { renderProviderCommand } from "../providers/agentProviderConfig.js";
...
class FakeWorkerAdapter implements WorkerAdapter {
  ...
  constructor(
    ...
    private readonly command = "codexyoloproxy",
    readonly kind: WorkerAdapter["kind"] = "codex"
  ) {}
...
describe("StewardRuntime", () => {
  it("uses the selected Worker provider command, model, and metadata for local dispatch", async () => {
    const dir = await mkdtemp(join(tmpdir(), "agent-fleet-steward-"));
    const codexAdapter = new FakeWorkerAdapter();
    const selectedProvider = {
      id: "gemini-review",
      type: "gemini_cli" as const,
      roles: ["worker" as const],
      commandProfiles: {
        default: { commandTemplate: "gemini --model {model} --provider {providerId}" }
      },
    ...
    try {
      ...
      const rendered = renderProviderCommand(selectedProvider);
      const runtime = new StewardRuntime({
        store,
        workerAdapter: codexAdapter,
        defaultWorkerCwd: "/worktrees/agent-fleet",
        workerProviderSelector: (input) =>
          input.placement === "local" && input.requiredTags.includes("high-cpu") ? selectedProvider : null,
        workerAdapterFactory: (selection) => {
          expect(selection.provider).toEqual(selectedProvider);
          expect(selection.renderedCommand).toEqual(rendered);
          return new FakeWorkerAdapter(undefined, "gemini-start", undefined, "running", rendered.displayCommand, "gemini_cli");
        }
      });
      ...
      await runtime.acceptGoal({
        ...
        title: "Run high CPU review",
        body: "Run high CPU review locally when no remote node is available."
      });

      const dashboard = await store.dashboard();

      expect(codexAdapter.startInputs).toHaveLength(0);
      expect(dashboard.workerSessions[0]).toMatchObject({
        kind: "gemini_cli",
        providerId: "gemini-review",
        providerType: "gemini_cli",
        model: "gemini-2.5-pro",
        command: rendered.displayCommand
      });
      expect(dashboard.decisions[0].actionsJson).toContain("Worker provider: gemini-review");
      expect(dashboard.decisions[0].actionsJson).toContain("Model: gemini-2.5-pro");
    } finally {
  ...
  it("acquires and installs a deploy-key lease before remote provisioning and injects git ssh env", async () => {
    ...
    try {
      ...
      expect(keyProvisioner.installInputs).toEqual([{ lease, sshHost: "worker@linux-builder.example" }]);
  ...
  it("selects a GPU-tagged ready remote node for GPU goals", async () => {
    ...
    try {
      ...
      await store.upsertExecutionNode({
        ...
        kind: "remote",
        status: "ready",
        sshHost: "worker@linux-cpu.example",
        workRoot: "/srv/cpu",
        proxyUrl: null,
        tags: ["remote", "linux", "high-cpu"],
        capacity: 2
      ...
      const gpuNode = await store.upsertExecutionNode({
        ...
        kind: "remote",
        status: "ready",
        sshHost: "worker@linux-gpu.example",
        workRoot: "/srv/gpu",
        proxyUrl: null,
  ...
  it("skips ready remote nodes at capacity and falls back to the next available remote", async () => {
    ...
    try {
      ...
      const availableNode = await store.upsertExecutionNode({
        ...
        kind: "remote",
        status: "ready",
        sshHost: "worker@available.example",
  ...
  it("turns remote execution corrections into reusable structured memory", async () => {
    ...
    try {
      ...
      const correction = await runtime.correctDecision({
        decisionId: decision.id,
        body: "For long/high-load tasks, route Workers to remote server remote-worker-01 and include -remote- in Worker names."
      });
