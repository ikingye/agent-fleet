...
import {
  ...
  type RenderedProviderCommand,
  type WorkerProviderSelectionInput
...
export interface StewardRuntimeOptions {
  ...
  workerProviderSelector?: (input: WorkerProviderSelectionInput) => WorkerProviderConfig | null;
  workerAdapterFactory?: (selection: RuntimeWorkerProviderSelection) => WorkerAdapter;
...
export interface RuntimeWorkerProviderSelection {
  provider: WorkerProviderConfig;
  renderedCommand: RenderedProviderCommand;
  kind: WorkerAdapter["kind"];
}

interface WorkerPlacement {
  adapter: WorkerAdapter;
  providerSelection: RuntimeWorkerProviderSelection | null;
  hostId: string | null;
  cwd: string;
  worktreePath: string;
  workerName: string;
...
export class StewardRuntime {
  ...
  async acceptGoal(input: AcceptGoalInput): Promise<Goal> {
    ...
    const decision = await this.options.store.recordDecision({
      ...
      actions: [
        ...
        ...buildProviderActions(placement.providerSelection),
    ...
    if (placement.hostId !== null) {
      remoteSession = await this.options.store.createWorkerSession({
        ...
        providerId: placement.providerSelection?.provider.id,
        providerType: placement.providerSelection?.provider.type,
        model: placement.providerSelection?.renderedCommand.model,
  ...
  private async selectWorkerPlacement(goal: Goal, workspacePath: string): Promise<WorkerPlacement> {
    ...
    if (remoteNode !== null && this.options.remoteWorkerAdapterFactory !== undefined) {
      const providerSelection = this.selectWorkerProvider({
        placement: "remote",
        requiredTags: resourceTags
      });

      if (this.options.workerProviderSelector === undefined || providerSelection !== null) {
        const remoteWorkspacePath = buildRemoteWorkspacePath(remoteNode, goal.projectName, workspacePath);

        return {
          adapter: this.options.remoteWorkerAdapterFactory(remoteNode, providerSelection),
          providerSelection,
    ...
    return {
      adapter:
        providerSelection === null || this.options.workerAdapterFactory === undefined
          ? this.options.workerAdapter
          : this.options.workerAdapterFactory(providerSelection),
      providerSelection,
      hostId: null,
      cwd: workspacePath,
  ...
  private selectWorkerProvider(input: WorkerProviderSelectionInput): RuntimeWorkerProviderSelection | null {
    const provider = this.options.workerProviderSelector?.(input) ?? null;
    ...
    return {
      provider,
      renderedCommand: renderProviderCommand(provider),
      kind: workerKindForProviderType(provider.type)
    };
  }
...
function buildProviderActions(selection: RuntimeWorkerProviderSelection | null): string[] {
  if (selection === null) {
    return [];
  }

  return [
    `Worker provider: ${selection.provider.id}`,
    `Provider type: ${selection.provider.type}`,
    ...(selection.renderedCommand.model === null ? [] : [`Model: ${selection.renderedCommand.model}`])
  ];
}
