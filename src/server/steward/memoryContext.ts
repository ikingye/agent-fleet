...
export function correctionMemoryKeys(body: string): string[] {
  ...
  if (/(long|high-load|high load|cpu|gpu|heavy|remote|aicp-hhht-231)/.test(normalized) && /remote|server|node|offload/.test(normalized)) {
  ...
  if (/\/users\/yewang|avoid.*ui|dashboard|owner-facing ui|owner facing ui/.test(normalized)) {
