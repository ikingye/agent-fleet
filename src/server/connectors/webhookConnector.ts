  ...
  return {
    ...
    async receiveMessage(input: WebhookConnectorReceiveInput): Promise<WebhookConnectorReply> {
      ...
      if (config.allowedSenderIds.length === 0) {
        throw new WebhookConnectorError(403, "Webhook connector sender allowlist is empty");
      }

      if (!config.allowedSenderIds.includes(message.senderId)) {
        throw new WebhookConnectorError(403, "Webhook connector sender is not allowed");
      }

      const stewardResult = await options.conversationService.acceptOwnerMessage({
        conversationId: webhookConversationId(config, message),
        transport: "im",
        externalMessageId: message.externalMessageId,
        idempotencyKey: message.idempotencyKey ?? message.externalMessageId ?? requestSignatureId(input),
      ...
      return {
        ...
        duplicate: stewardResult.duplicate,
        messageType: "text",
...
function parseInboundWebhookMessage(rawBody: string): InboundWebhookMessage {
  ...
  const externalMessageId = optionalString(
    body.externalMessageId ?? body.messageId ?? body.message_id ?? body.msgId ?? body.MsgId ?? body.id
  );
...
function webhookConversationId(config: WebhookConnectorConfig, message: InboundWebhookMessage): string {
  return `webhook:${config.id}:${message.conversationId ?? message.senderId}`;
}

function requestSignatureId(input: WebhookConnectorReceiveInput): string | null {
  const signature = optionalString(input.query.signature) ?? optionalString(input.headers["x-agent-fleet-signature"]);
