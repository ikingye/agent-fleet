...
import { defaultAgentFleetConfig, saveAgentFleetConfig } from "../providers/agentProviderConfig.js";
...
describe("API routes", () => {
  ...
  it("exposes and replaces provider config through the API", async () => {
    const app = await createApp({
      statePath: join(dir, "state.json"),
    ...
    try {
      const initialResponse = await app.inject({ method: "GET", url: "/api/providers" });
      ...
      expect(initialResponse.json().config.workerProviders[0]).toMatchObject({
        id: "codexyoloproxy",
        commandProfiles: { default: { commandTemplate: "codexyoloproxy" } }
      });
      ...
      const updatedResponse = await app.inject({ method: "GET", url: "/api/providers" });

      expect(updateResponse.statusCode).toBe(200);
      expect(updatedResponse.json().config.workerProviders).toEqual([
  ...
  it("uses provider config to render the Worker command when no explicit adapter is supplied", async () => {
    const configPath = join(dir, "config.json");
    await saveAgentFleetConfig(configPath, {
      version: 1,
      stewardProviderId: "codexyoloproxy",
      stewardProvider: defaultAgentFleetConfig().stewardProvider,
      workerProviders: [
        {
          id: "node-provider",
          type: "custom",
          ...
          commandProfiles: {
            default: {
              commandTemplate: `${process.execPath} -e "if (process.argv[1] !== 'provider-model') { process.exit(1); } process.stdin.resume(); process.stdin.on('end', () => console.log('resume id: provider-config'));" {model}`
            }
          },
          defaultCommandProfile: "default",
    ...
    try {
      const goalResponse = await app.inject({
        ...
        payload: {
          projectName: "agent-fleet",
          workspacePath: dir,
          title: "Use provider config",
          body: "Launch the configured provider."
        }
      });
      ...
      expect(dashboard.workerSessions[0]).toMatchObject({
        ...
        resumeId: "provider-config",
        status: "completed"
      });
      expect(dashboard.workerSessions[0].command).toContain("provider-model");
    } finally {
      await app.close();
    }
  });

  it("records human provider labels in Worker session metadata through the API path", async () => {
    for (const providerType of ["claude", "gemini"] as const) {
      const configPath = join(dir, `${providerType}-config.json`);
      await saveAgentFleetConfig(configPath, {
        version: 1,
        stewardProviderId: "codexyoloproxy",
        stewardProvider: defaultAgentFleetConfig().stewardProvider,
        workerProviders: [
          {
            id: `${providerType}-review`,
            type: providerType,
            roles: ["worker"],
            commandProfiles: {
              default: {
                commandTemplate: `${process.execPath} -e "if (process.argv[1] !== '${providerType}') { process.exit(1); } process.stdin.resume(); process.stdin.on('end', () => console.log('resume id: ${providerType}-provider-config'));" {providerType}`
              }
            },
            defaultCommandProfile: "default",
            defaultModel: null,
      ...
      const app = await createApp({
        statePath: join(dir, `${providerType}-state.json`),
        configPath,
        defaultWorkerCwd: "/worktrees/agent-fleet"
      });
      ...
      try {
        const goalResponse = await app.inject({
          ...
          payload: {
            projectName: "agent-fleet",
            workspacePath: dir,
            title: `Use ${providerType} provider config`,
            body: "Launch the configured provider."
          }
        });
        const dashboard = (await app.inject({ method: "GET", url: "/api/dashboard" })).json();
        ...
        expect(dashboard.workerSessions[0]).toMatchObject({
          kind: providerType,
          providerId: `${providerType}-review`,
          providerType,
          resumeId: `${providerType}-provider-config`,
          status: "completed"
        });
      } finally {
        await app.close();
  ...
  it("records owner Steward chat messages and persists deterministic Steward responses", async () => {
    ...
    try {
      ...
      expect(messageResponse.json().stewardMessage.body).toContain("/workspaces/mahjong");
      expect(messageResponse.json().stewardMessage.body).toContain("1 active goal");

      const listResponse = await app.inject({
        method: "GET",
        url: "/api/steward/messages?workspacePath=%2Fworkspaces%2Fmahjong"
      });
      const dashboard = (await app.inject({ method: "GET", url: "/api/dashboard" })).json();
  ...
  it("receives signed webhook connector messages through the Steward conversation API", async () => {
    ...
    try {
      ...
      expect(response.json().text).toContain("/workspaces/mahjong");
  ...
  it("reconciles remote Worker sessions through the execution node sshHost", async () => {
    ...
    try {
      ...
      expect(dashboard.workerSessions[0]).toMatchObject({
        ...
        lastOutput: "remote pid 7777 is no longer running on remote-worker-01"
      });
      expect(dashboard.goals[0]).toMatchObject({ status: "blocked" });
  ...
  it("reconciles a remote Worker without a resume id to failed and blocks the goal", async () => {
    ...
    try {
      ...
      expect(dashboard.workerSessions[0]).toMatchObject({
        ...
        status: "failed",
        resumeId: null,
        lastOutput: "remote pid 8888 is no longer running on remote-worker-02"
      });
      expect(dashboard.goals[0]).toMatchObject({ status: "blocked" });
    ...
    try {
      const defaultedResponse = await app.inject({
        ...
        payload: {
          ...
          kind: "remote",
          status: "unknown",
          sshHost: "worker@remote-worker-01",
          proxyUrl: null
        }
      });
      const normalizedResponse = await app.inject({
        ...
        payload: {
          ...
          kind: "remote",
          status: "unknown",
          sshHost: "worker@remote-worker-02",
          workRoot: "/tmp/agent-fleet/work/",
          proxyUrl: null
        }
      });
      const rootResponse = await app.inject({
        ...
        url: "/api/execution-nodes",
        payload: {
          name: "remote-worker-03",
          kind: "remote",
          status: "unknown",
          sshHost: "worker@remote-worker-03",
      ...
      const customScratchResponse = await app.inject({
        ...
        url: "/api/execution-nodes",
        payload: {
          name: "remote-worker-04",
          kind: "remote",
          status: "unknown",
          sshHost: "worker@remote-worker-04",
    ...
    try {
      const nodeResponse = await app.inject({
        ...
        payload: {
          ...
          sshHost: "worker@linux-builder.example",
          workRoot: "/tmp/agent-fleet",
          proxyUrl: "http://proxy-credential-marker@127.0.0.1:1080"
        }
      });
      const node = nodeResponse.json();
      ...
      expect(JSON.stringify(onboardingResponse.json())).not.toContain("proxy-credential-marker");
      expect(remoteRunner.inputs[0].remoteScript).not.toContain("proxy-credential-marker");
    } finally {
      await app.close();
    }
  });
  ...
  it("dispatches accepted goals to a ready remote execution node through SSH", async () => {
    ...
    try {
      ...
      expect(sshRunner.inputs[0].args.slice(0, -1)).toEqual(["remote-worker-01"]);
      ...
      expect(dashboard.workerSessions[0]).toMatchObject({
        hostId: node.id,
        command: "ssh remote-worker-01 fake-remote-worker --safe-smoke",
