...
} from "../providers/agentProviderConfig.js";
...
import { StewardRuntime, type RuntimeWorkerProviderSelection } from "../steward/stewardRuntime.js";
...
export interface CreateAppOptions {
  statePath?: string;
  configPath?: string;
  providerConfig?: AgentFleetConfig;
  ...
  remoteWorkerAdapterFactory?: (node: ExecutionNode, selection: RuntimeWorkerProviderSelection | null) => WorkerAdapter;
...
function legacyWorkerConfigPresent(options: CreateAppOptions): boolean {
  return (
    options.workerCommand !== undefined ||
    options.workerArgs !== undefined ||
    process.env.AGENT_FLEET_WORKER_COMMAND !== undefined ||
...
export async function createApp(options: CreateAppOptions = {}) {
  ...
  const configPath = options.configPath ?? agentFleetConfigPath(process.cwd(), process.env);
  let providerConfig = normalizeAgentFleetConfig(
    options.providerConfig ?? (await readAgentFleetConfig(configPath)) ?? defaultAgentFleetConfig()
  );
  const workerArgs = options.workerArgs ?? parseWorkerCommandArgs(process.env.AGENT_FLEET_WORKER_ARGS);
  const workerCommand = options.workerCommand ?? process.env.AGENT_FLEET_WORKER_COMMAND ?? "codexyoloproxy";
  if (legacyWorkerConfigPresent(options)) {
    const legacyProvider = providerFromLegacyWorkerCommand(workerCommand, workerArgs);
    providerConfig = {
      ...providerConfig,
      workerProviders: [
        legacyProvider,
        ...providerConfig.workerProviders.filter((provider) => provider.id !== legacyProvider.id)
      ]
    };
  }
  const defaultWorkerProvider =
    selectWorkerProvider(providerConfig, { placement: "local", requiredTags: [] }) ??
    providerFromLegacyWorkerCommand(workerCommand, workerArgs);
  const defaultWorkerCommand = renderProviderCommand(defaultWorkerProvider);
  const defaultWorkerKind = workerKindForProviderType(defaultWorkerProvider.type);
  const useConfiguredProviderAdapters = options.workerAdapter === undefined;
  const currentProbeWorkerCommand = () => {
    const provider =
      selectWorkerProvider(providerConfig, { placement: "remote", requiredTags: [] }) ??
      selectWorkerProvider(providerConfig, { placement: "local", requiredTags: [] }) ??
      defaultWorkerProvider;

    return renderProviderCommand(provider).command;
  };
  const workerAdapter =
    options.workerAdapter ??
    new CommandWorkerAdapter(defaultWorkerCommand.command, defaultWorkerCommand.args, undefined, process.env, defaultWorkerKind);
  ...
  const steward = new StewardRuntime({
    store,
    workerAdapter,
    workerProviderSelector: useConfiguredProviderAdapters
      ? (input) => selectWorkerProvider(providerConfig, input)
      : undefined,
    workerAdapterFactory: useConfiguredProviderAdapters
      ? (selection) =>
          new CommandWorkerAdapter(
            selection.renderedCommand.command,
            selection.renderedCommand.args,
            undefined,
            process.env,
            selection.kind
          )
    ...
    remoteWorkerAdapterFactory:
      ...
      ((node, selection) => {
        const renderedCommand = selection?.renderedCommand ?? defaultWorkerCommand;

        return new RemoteSshWorkerAdapter({
          kind: selection?.kind ?? defaultWorkerKind,
          sshHost: node.sshHost ?? "",
          workerCommand: renderedCommand.command,
          workerArgs: renderedCommand.args,
  ...
  app.get("/api/providers", async () => ({ config: providerConfig }));

  app.put("/api/providers", async (request, reply) => {
    const body = requestBody(request.body);

    try {
      providerConfig = await saveAgentFleetConfig(configPath, normalizeAgentFleetConfig(body.config ?? body));
  ...
  app.post("/api/execution-nodes/:id/probe", async (request, reply) => {
    ...
    try {
      return await probeRemoteExecutionNode({
        node,
        codexCommand: currentProbeWorkerCommand(),
