...
describe("runRemoteNodeOnboarding", () => {
  it("normalizes remote work roots to /tmp/agent-fleet/work", async () => {
    ...
    expect(runner.inputs[0]).toMatchObject({ sshHost: "worker@builder.example" });
    ...
    expect(runner.inputs[0].remoteScript).not.toContain("proxy-credential-marker");
    expect(JSON.stringify(result)).not.toContain("proxy-credential-marker");
  });
  ...
  it("turns ssh runner failure into a sanitized ssh reachability failure", async () => {
    const runner = new CapturingRemoteRunner({
      exitCode: 255,
      stdout: "",
      stderr: "Permission denied (publickey). proxy-credential-marker"
    });
