...
Examples use placeholders such as `remote-worker-01`, `worker`, and `203.0.113.10`. Keep real hostnames, IP addresses, account names, and SSH config in ignored local operator state.
...
Register as many remote nodes as are available. `remote-worker-02` is one SSH config host example, not a singleton or special project:
...
Only add the `gpu` tag after a read-only probe confirms GPU capability, for example `ssh remote-worker-01 'command -v nvidia-smi && nvidia-smi -L'`. Do not infer GPU capability from the host name.
