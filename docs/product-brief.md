...
- Clear Worker identity: every Worker task has a human-readable Worker Name in the format `<project-name>-<worker-purpose>-<worker-start-time-string>`, such as `agent-fleet-compact-dashboard-ui-20260426T164821+0800` or `mahjong-project-readiness-20260426T164821+0800`. The Steward puts that name at the top of the Worker prompt and requires the Worker to use the exact name as the heading of its final report. Random spawn-system nicknames are secondary; the explicit Worker Name is the source of truth.
